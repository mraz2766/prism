...
Prism 采用自适应出口观察：网络稳定时目标节拍为 1 秒；出现候选地址、系统网络变化、唤醒或手动刷新后，在 5 秒内提升为 250ms 确认节拍。IPv4 与 IPv6 并发观察，请求不并发堆积。两种地址族在同一轮共享连接，但连接代际最多保留 1.5 秒，避免 OneBox 等代理工具切换国家节点后长期沿用旧隧道；相比每个请求都创建会话，也能显著降低后台 CPU。第一次不同路线的候选会立即显示“正在确认”；连续两次得到相同路线后才提交。同一代理路线内出现新 IP 会立即提交。单次海外请求失败、候选撤销或迟到的旧请求不会覆盖当前稳定出口。
...
- 小改动：次版本加 `0.1`，例如 `1.0.0 → 1.1.0`
- 大改动：主版本加 `1` 并将次版本归零，例如 `1.2.0 → 2.0.0`

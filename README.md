...
- 可在设置中滑动调节检测灵敏度；灵敏度越高，越快发现 VPN 不可用、恢复和地区切换
- 根据灵敏度以 2–12 秒检查稳定网络；低电量模式下自动降低频率
- 发现候选地址、探测失败、网络变化、唤醒或手动刷新后，短暂切换到 100–500ms 确认节拍
- 连续探测失败时及时标记出口不可用，探测恢复后即使 IP 未变也会恢复实时状态
